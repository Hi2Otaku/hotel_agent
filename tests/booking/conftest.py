...
# Ensure booking's app package takes precedence.
# Must be inserted BEFORE clearing modules so Python resolves 'app' to booking.
_booking_root = str(Path(__file__).resolve().parent.parent.parent / "services" / "booking")

# Remove ALL paths to other services' app packages to prevent collision
sys.path = [
    p for p in sys.path
    if not (
        p.endswith("services/auth") or p.endswith("services\\auth")
        or p.endswith("services/room") or p.endswith("services\\room")
    )
]
if _booking_root not in sys.path:
    sys.path.insert(0, _booking_root)

# Remove cached 'app' modules from other services so booking's app is loaded
_to_remove = [key for key in sys.modules if key == "app" or key.startswith("app.")]
for key in _to_remove:
    del sys.modules[key]

# Clear shared Base metadata to avoid conflicts from other services' models
from shared.database import Base as _SharedBase  # noqa: E402
_SharedBase.metadata.clear()

from app.api.deps import get_current_user, get_db  # noqa: E402
from app.core.database import Base  # noqa: E402
from app.main import app  # noqa: E402
from app.services import booking as _booking_service_module  # noqa: E402
