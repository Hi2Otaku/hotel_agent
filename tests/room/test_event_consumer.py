...
def _ensure_room_imports():
    """Ensure room service modules are importable by inserting service path."""
    import os
    room_path = os.path.join(os.path.dirname(__file__), "..", "..", "services", "room")
    room_path = os.path.abspath(room_path)
    shared_path = os.path.join(os.path.dirname(__file__), "..", "..", "shared")
    shared_path = os.path.abspath(shared_path)

    # Remove auth app from sys.modules to allow room app imports
    # Save and restore after -- but for test purposes we just need the model
    if room_path not in sys.path:
